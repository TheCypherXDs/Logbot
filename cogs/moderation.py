...
class ModerationCog(commands.Cog):
    ...
    @app_commands.command(name='clean', description='Очищает сообщения')
    ...
    async def clean(self, interaction: discord.Interaction, amount: app_commands.Range[int, 1, 100]):
        ...
        try:
            ...
            await self.log_mod_action(interaction.guild, "🧹 Очистка чата", f"Удалено {len(deleted)} сообщений в {interaction.channel.mention}")
    ...
    async def warn(self, interaction: discord.Interaction, member: discord.Member, reason: str):
        ...
        await self.log_mod_action(interaction.guild, "⚠️ Выдано предупреждение", f"Пользователь: {member.mention}\nПричина: {reason}")
    ...
    async def warnings(self, interaction: discord.Interaction, member: discord.Member):
        ...
        if not warns:
            return await interaction.followup.send(f"У {member.mention} нет предупреждений.", ephemeral=True)
    ...
    @app_commands.command(name='unwarn', description='Удалить предупреждение')
    @app_commands.default_permissions(moderate_members=True)
    ...
    async def setlogchannel(self, interaction: discord.Interaction, 
        ...
        await interaction.response.send_message(f"✅ Канал для `{type}` установлен: {channel.mention}", ephemeral=True)

    @app_commands.command(name='settings', description='Текущие настройки логов')
    @app_commands.default_permissions(administrator=True)
    async def settings(self, interaction: discord.Interaction):
        ...
        if not settings: embed.description = "Настройки еще не заданы."
    ...
    @app_commands.command(name='userlog', description='Последние действия пользователя')
    @app_commands.default_permissions(moderate_members=True)
    async def userlog(self, interaction: discord.Interaction, member: discord.Member):
        ...
        if not logs:
            return await interaction.followup.send(f"Действий {member.mention} не найдено.", ephemeral=True)
    ...
    async def searchlogs(self, interaction: discord.Interaction, 
        ...
        if not results:
            return await interaction.followup.send("Ничего не найдено.", ephemeral=True)
    ...
    async def modstats(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="Действий в логах", value=action_text, inline=False)
        await interaction.followup.send(embed=embed, ephemeral=True)
